...
async function testIdempotency() {
    ...
    await runTest('1. Initial Request (Expect 200)', async () => {
        ...
        if (data.message !== 'Charged 100 USD')
    ...
    await runTest('2. Duplicate Request (Expect 200 + Cache Hit)', async () => {
        ...
        if (res.status !== 200)
            throw new Error(`Expected 200, got ${res.status}`);
        if (res.headers.get('x-cache-hit') !== 'true')
            throw new Error('Missing X-Cache-Hit header');
        const data = await res.json();
        if (data.message !== 'Charged 100 USD')
            throw new Error(`Unexpected body: ${JSON.stringify(data)}`);
    });
    ...
    await runTest('4. Concurrent Requests (Expect In-Progress Handling)', async () => {
        const concurKey = crypto_1.default.randomUUID();
        ...
        if (r1.status !== 200)
            throw new Error(`Req 1 failed with ${r1.status}`);
        if (r2.status !== 200)
            throw new Error(`Req 2 failed with ${r2.status}`);
        const t1 = await r1.json();
        const t2 = await r2.json();
        if (t1.message !== 'Charged 100 USD')
            throw new Error('Req 1 body mismatch');
        if (t2.message !== 'Charged 100 USD')
            throw new Error('Req 2 body mismatch');
        if (r2.headers.get('x-cache-hit') !== 'true')
            throw new Error('Req 2 should be cache hit (waited for result)');
    });
    // 5. Invalid Payload
    ...
    await runTest('6. Server Health Check (Verify Up)', async () => {
        ...
        if (res.status !== 200)
            throw new Error(`Server seems down, got ${res.status}`);
    });
}
